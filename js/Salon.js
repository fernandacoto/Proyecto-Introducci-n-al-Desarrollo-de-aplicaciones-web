...
{
	sesion = document.getElementById("NombreSalon");
    ...
    if (sesion_valor && descripcion_valor)
    {
		...
		xmlhttp.open("GET","agregarSalon.php?NombreSalon=" + document.getElementById("NombreSalon").value + "&DescripcionSalon=" + document.getElementById("DescripcionSalon").value,true);
		xmlhttp.send();
...
{
	window.location.replace('http://ic-itcr.ac.cr/~jsanchez_/Proyecto/UsuarioAdministrador/CrearSalon.php');
}
...
function borrarSalon(IdSalon)
{
	...
	xmlhttp.open("GET","borrarSalon.php?IdSalon=" + IdSalon,true);
	xmlhttp.send();
